function [eout,thresh,gv_45,gh_135] = CEdge(varargin)
...
elseif any(strcmp(method, {'log','zerocross'}))
    ...
    if isPrewittOrSobel
        ...
        [bx, by, b] = MATLAB.DataTypes.ArrayFun(@(a)images.internal.builtins.edgesobelprewitt(a, isSobel, kx, ky),a,Dimension=1:2,CatMode=MATLAB.Flags.EsNlcs);
    ...
    if isempty(thresh) % Determine cutoff based on RMS estimate of noise
        ...
        cutoff = scale * mean(b,1:2,'double');
        thresh = sqrt(cutoff);
    else
    ...
    if thinning
        e = MATLAB.DataTypes.ArrayFun(@(b,bx,by,cutoff)images.internal.builtins.computeEdges(b,bx,by,kx,ky,int8(offset),100*eps,cutoff),b,bx,by,cutoff);
    else
        e = b > cutoff;
    end
...
validateattributes(I,{'numeric','logical'},{'real','nonsparse'},mfilename,'I',1);
